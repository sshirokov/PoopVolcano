...
Twitter.prototype.handlers = {
    ...
    tag: function(user, tagged) {
        var self = this,
            template = "Ding! @{user} just tagged {people}",
            ctx = {
        ...
        this.connection.updateStatus(
                template.$format(ctx),
                function(err, data) {
                    if(!data.id) self.handlers._error('tag', err);
                }
        );
    },
    ...
    poop: function(data) {
        ...
        var argv = require('optimist').default('data',"{0}/data".$format(process.cwd())).argv,
            reply = require("helpers").path.readJSON(argv.data + "/responses.json");
        ...
        this.connection.newDirectMessage(
            data.from.id, reply,
            function(err, data) {
                if(!data.id) self.handlers._error('poop', err);
            }
        );
    ...
    direct_message: function(data) {
        ...
        ctx.message = data.text.replace(/^\s+|\s+$/g,"").slice(0, 140 - template.$format(ctx).length);
        ...
        if(ctx.message.length < 2)
            ...
            this.connection.updateStatus(
                template.$format(ctx),
                function(err, data) {
                    if(!data.id) self.handlers._error('direct_message', err);
                }
            );
        return this;
    },
